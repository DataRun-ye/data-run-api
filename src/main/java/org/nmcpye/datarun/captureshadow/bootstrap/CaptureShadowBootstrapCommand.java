...
@ConditionalOnProperty(
    name = {
        CaptureShadowBootstrapCommand.ENABLED_PROPERTY,
        CaptureShadowBootstrapCommand.ISOLATED_PROPERTY,
        CaptureShadowBootstrapCommand.NO_WRITERS_PROPERTY
    },
    havingValue = "true"
)
public class CaptureShadowBootstrapCommand implements ApplicationRunner {

    public static final String ENABLED_PROPERTY = "datarun.capture-shadow.bootstrap.enabled";
    public static final String ISOLATED_PROPERTY = "datarun.capture-shadow.bootstrap.isolated";
    public static final String NO_WRITERS_PROPERTY = "datarun.capture-shadow.bootstrap.no-writers";

    private final CaptureShadowBootstrap bootstrap;
    private final ConfigurableApplicationContext applicationContext;
    private final Environment environment;

    public CaptureShadowBootstrapCommand(
        CaptureShadowBootstrap bootstrap,
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            System.out.print(bootstrap.run().toOperatorText());
        } catch (CaptureShadowBootstrapMismatchException exception) {
            System.err.print(exception.report().toOperatorText());
            throw exception;
        } catch (RuntimeException exception) {
            System.err.println("capture-shadow-bootstrap/v1");
