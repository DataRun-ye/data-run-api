...
public abstract class OrgUnitSpecifications extends DefaultIdentifiableSpecifications<OrgUnit> {
    public OrgUnitSpecifications(IdentifiableRelationalRepository<OrgUnit> repository) {
        super(repository);
    }

    public Specification<OrgUnit> hasLevel(Integer level) {
        return (root, query, criteriaBuilder) -> level == null ? null : criteriaBuilder.equal(root.get("level"), level);
    }

    public Specification<OrgUnit> hasParent(String parent) {
