...
public class OrgUnitFilter extends DefaultJpaFilter<OrgUnit> {
    ...
    public Specification<OrgUnit> getAccessSpecification(CurrentUserDetails user,
        ...
        final Set<String> ancestorsUids = directOrgUnits.stream()
            .flatMap(o -> o.getAncestorUids(null).stream()).collect(Collectors.toSet());

        final var allUids = Stream.concat(ancestorsUids.stream(), directAndManagedUids.stream()).collect(Collectors.toSet());
...
//                    teamNotDisabled);
    ...
    Set<OrgUnit> getDirectAndManagedOrgUnit(CurrentUserDetails user) {
        final var orgUnitSet = assignmentRepository.findAllByTeamUidIn(
                Stream.concat(user.getUserTeamIds().stream(),
                        user.getUserManagedTeamIds().stream())
