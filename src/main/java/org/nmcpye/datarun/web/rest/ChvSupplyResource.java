...
public class ChvSupplyResource {
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<ChvSupply>> getAllChvSupplies(
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        @RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload
    ) {
        log.debug("REST request to get a page of ChvSupplies");
        Page<ChvSupply> page;
        if (eagerload) {
            page = chvSupplyService.findAllWithEagerRelationships(pageable);
        } else {
            page = chvSupplyService.findAll(pageable);
        }
