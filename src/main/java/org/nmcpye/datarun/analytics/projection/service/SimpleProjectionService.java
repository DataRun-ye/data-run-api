...
//    private final ApplicationContext appCtx;    // for self-proxy calls
...
//
//    /**
//     * High-level entry: process a repeat for multiple semantic paths (iterates pages and calls the transactional batch method).
//     * - repeatUid: RPT_...
//     * - semanticPaths: list (non-empty). If you want to auto-discover call getSemanticPathsForRepeat(...) from PilotRunner and pass list.
//     */
//    @Override
//    public void processRepeatForPaths(String repeatUid, List<String> semanticPaths, int pageSize) throws Exception {
//        if (semanticPaths == null || semanticPaths.isEmpty()) {
//            log.warn("No semantic paths provided for repeatUid {}", repeatUid);
//            return;
//        }
//        for (String semanticPath : semanticPaths) {
//            int offset = 0;
//            while (true) {
//                List<Map<String, Object>> rows = fetchRawRepeatPayloadBatch(repeatUid, semanticPath, pageSize, offset);
//                if (rows.isEmpty()) break;
//
//                // Use Spring proxy to ensure transactional method is used
//                SimpleProjectionService self = appCtx.getBean(SimpleProjectionService.class);
//                self.processBatchTransactional(loadProjectionConfigForRepeat(repeatUid), rows, repeatUid, semanticPath);
//
//                offset += rows.size();
...
//
//    /**
//     * Public transactional per-batch processor.  Each call is its own transaction.
//     * It dispatches to the private processors (supply, breeding, etc.) based on the projection target table.
//     */
//    @Transactional
//    public void processBatchTransactional(JsonNode projectionConfig, List<Map<String, Object>> rows, String repeatUid, String semanticPath) throws Exception {
//        // Determine target table from projectionConfig (must exist for mapped repeats)
//        String targetTable = null;
//        if (projectionConfig != null && projectionConfig.has("target")
//            && projectionConfig.get("target").has("table")) {
//            targetTable = projectionConfig.get("target").get("table").asText();
//        }
//
//        if ("supply_event".equalsIgnoreCase(targetTable) || "supply_event".equalsIgnoreCase(repeatUid)) {
//            // call your existing private supply batch processor
//            processSupplyBatch(projectionConfig, rows, repeatUid);
//        } else if ("breeding_source_event".equalsIgnoreCase(targetTable) || "breeding_source_event".equalsIgnoreCase(repeatUid)) {
//            processBreedingBatch(projectionConfig, rows, repeatUid);
//        } else {
//            // fallback: try to infer by repeatUid or throw
//            LOG.info("No explicit target in projectionConfig for repeatUid={}, semanticPath={}, trying default processor dispatch", repeatUid, semanticPath);
//            // You can add more heuristics here (e.g. map repeatUid => table via a small map)
//            throw new IllegalStateException("Unknown target table for repeatUid " + repeatUid + ". Provide projection_config entry.");
//        }
//    }
//
//    /**
//     * Fetch a single page of raw_repeat_payload rows for a given repeat_uid + semantic_path.
//     * Returns List<Map<String,Object>> where each map contains:
//     *  - "id" (UUID), "payload" (JsonNode), "submission_uid", "template_version_uid",
//     *  "occurrence_index", "projection_run_id", "payload_checksum" ...
//     */
//    private List<Map<String, Object>> fetchRawRepeatPayloadBatch(String repeatUid, String semanticPath, int pageSize, int offset) {
//        final String sql = ""
//            + "SELECT id, payload, (payload->>'_id') AS payload_id, submission_uid, template_uid, template_version_uid, occurrence_index, payload_checksum, projection_run_id "
//            + "FROM raw_repeat_payload "
//            + "WHERE repeat_uid = :repeatUid AND semantic_path = :semanticPath "
//            + "ORDER BY extracted_at, id "
//            + "LIMIT :limit OFFSET :offset";
//
//        MapSqlParameterSource params = new MapSqlParameterSource()
//            .addValue("repeatUid", repeatUid)
//            .addValue("semanticPath", semanticPath)
...
//            try {
//                payloadNode = payloadStr != null ? om.readTree(payloadStr) : null;
//            } catch (Exception e) {
//                LOG.warn("Failed parse payload json for raw_repeat_payload.id={} : {}", rs.getObject("id"), e.getMessage());
//            }
//            m.put("id", rs.getObject("id"));
//            m.put("payload", payloadNode);
//            m.put("payload_id", rs.getString("payload_id"));
//            m.put("submission_uid", rs.getString("submission_uid"));
//            m.put("template_uid", rs.getString("template_uid"));
//            m.put("template_version_uid", rs.getString("template_version_uid"));
//            m.put("occurrence_index", rs.getInt("occurrence_index"));
//            m.put("payload_checksum", rs.getString("payload_checksum"));
//            m.put("projection_run_id", rs.getString("projection_run_id"));
//            return m;
//        });
//
//        return rows;
...
//
//    /**
//     * Load projection_config JSON for this repeat if present (returns JsonNode or null).
//     * Uses projection_config.source_repeat_uid match.
//     */
//    private JsonNode loadProjectionConfigForRepeat(String repeatUid) {
//        try {
//            String sql = "SELECT payload FROM projection_config WHERE source_repeat_uid = :repeatUid LIMIT 1";
//            MapSqlParameterSource params = new MapSqlParameterSource().addValue("repeatUid", repeatUid);
//            String projectionJson = jdbc.queryForObject(sql, params, String.class);
//            if (projectionJson == null) return null;
//            return om.readTree(projectionJson);
//        } catch (EmptyResultDataAccessException e) {
//            return null;
//        } catch (Exception e) {
//            LOG.warn("Failed to load projection_config for repeatUid {}: {}", repeatUid, e.getMessage());
//            return null;
//        }
//    }
//
