...
public class PivotMetadataServiceImpl implements PivotMetadataService {
    ...
    public PivotMetadataResponse getMetadataForTemplate(String templateId, String templateVersionId) {
        ...
        // core dimensions (minimal set). For now, static set; you can populate more metadata from DB if desired
        List<PivotFieldDto> coreDimensions = List.of(
            PivotFieldDto.builder().id("team_id").label("Team").category("CORE_DIMENSION")
                .dataType("team_id").aggregationModes(Set.of("COUNT")).templateModeOnly(false).source("system").build(),
            PivotFieldDto.builder().id("org_unit_id").label("Org Unit").category("CORE_DIMENSION")
                .dataType("org_unit_id").aggregationModes(Set.of("COUNT")).templateModeOnly(false).source("system").build(),
            PivotFieldDto.builder().id("activity_id").label("Activity").category("CORE_DIMENSION")
                .dataType("activity_id").aggregationModes(Set.of("COUNT")).templateModeOnly(false).source("system").build(),
        ...
        return PivotMetadataResponse.builder()
            ...
            .measures(measures)
            .hints(Map.of("mode", "template", "templateId", templateId, "templateVersionId", templateVersionId))
            .build();
    }
