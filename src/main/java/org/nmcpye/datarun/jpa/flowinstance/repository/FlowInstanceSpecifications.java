 ...
 */
public abstract class FlowInstanceSpecifications {
    /**
     * <pre>{@code
     ...
     */
    public static Specification<FlowInstance> hasAncestors(List<Long> accessibleIds) {
        return (root, query, cb) -> {
            // Subquery to get all ancestor UIDs from accessible OrgUnits' paths
            Subquery<String> subquery = query.subquery(String.class);
            Root<FlowInstance> acc = subquery.from(FlowInstance.class);
            subquery.select(
    ...
    public static Specification<FlowInstance> hasDescendants(List<Long> accessibleIds) {
        return (root, query, cb) -> {
    ...
    //    @Override
    public static Specification<FlowInstance> canRead() {
        return (root, query, criteriaBuilder) -> {
            if (SecurityUtils.hasCurrentUserAnyOfAuthorities(AuthoritiesConstants.ADMIN)) {
                ...
                String currentUserLogin = SecurityUtils.getCurrentUserLoginOrThrow(
                    new ErrorMessage(ErrorCode.E3004, FlowInstance.class.getName()));
