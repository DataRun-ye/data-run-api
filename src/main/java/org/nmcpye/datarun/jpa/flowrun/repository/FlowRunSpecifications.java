...
public abstract class FlowRunSpecifications {
    /**
     * <pre>{@code
     ...
     */
    public static Specification<FlowRun> hasAncestors(List<Long> accessibleIds) {
        return (root, query, cb) -> {
            // Subquery to get all ancestor UIDs from accessible OrgUnits' paths
            Subquery<String> subquery = query.subquery(String.class);
            Root<FlowRun> acc = subquery.from(FlowRun.class);
            subquery.select(
    ...
    public static Specification<FlowRun> hasDescendants(List<Long> accessibleIds) {
        return (root, query, cb) -> {
    ...
    //    @Override
    public static Specification<FlowRun> canRead() {
        return (root, query, criteriaBuilder) -> {
            if (SecurityUtils.hasCurrentUserAnyOfAuthorities(AuthoritiesConstants.ADMIN)) {
                ...
                String currentUserLogin = SecurityUtils.getCurrentUserLoginOrThrow(
                    new ErrorMessage(ErrorCode.E3004, FlowRun.class.getName()));
                Join<FlowRun, Team> assignmentJoin = root.join("team", JoinType.INNER);
